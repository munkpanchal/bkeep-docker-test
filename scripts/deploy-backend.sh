...
IMAGE_NAME="${DOCKER_USERNAME}/docker-test-backend:latest"
CONTAINER_NAME="docker-test-backend-prod"
...
docker run -d \
  ...
  -e PORT=5000 \
  -e ALLOWED_ORIGINS=http://72.62.161.70,http://72.62.161.70:80,http://72.62.161.70:5001 \
  "$IMAGE_NAME"
...
while [ $WAIT_TIME -lt $MAX_WAIT ]; do
  if docker exec "$TEMP_CONTAINER" wget --quiet --tries=1 --spider http://localhost:5000/api/health 2>/dev/null; then
