...
const corsOptions = {
  origin: (
    ...
    // In production, be more permissive - allow same hostname (regardless of protocol/port)
    if (isProduction()) {
      try {
        const originUrl = new URL(origin);
        const frontendUrl = new URL(env.FRONTEND_URL);

        // Allow if hostname matches (ignore protocol and port)
        if (originUrl.hostname === frontendUrl.hostname) {
          logger.info(`CORS: Allowing same hostname: ${origin}`);
          return callback(null, true);
        }

        // Also check against allowed origins with hostname matching
        ...
        if (hostnameMatch) {
          logger.info(`CORS: Allowing hostname match: ${origin}`);
          return callback(null, true);
        }
      } catch {
